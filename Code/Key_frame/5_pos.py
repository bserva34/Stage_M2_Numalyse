...
def main():
    ...
    p.add_argument("--num", type=int, default=3, help="Nombre de keyframes à extraire sur le score combiné")
    p.add_argument("--final", type=int, default=1,
    ...
    for i in range(len(frames)):
        ...
        comb = 0.2 * norm_s + 0.7 * norm_d + 0.1*gaussienne[i]
    ...
    if args.final > 0:
        # 1. Calculer un score de qualité pour chaque frame
        #    on convertit en gris puis on calcule la variance du Laplacien (indicateur de netteté)
        quality_scores = []
        for img in frames:
            grey = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            lap = cv2.Laplacian(grey, cv2.CV_64F)
            quality_scores.append(lap.var())

        quality_scores = np.array(quality_scores, dtype=float)

        # 2. Normaliser ces scores pour qu'ils soient compris entre 0 et 1
        quality_weights = quality_scores / quality_scores.max()

        # 3. Ne conserver et pondérer que les frames de initial_top_idxs
        weighted = [
            (i, comb_scores[i] * quality_weights[i])
            for i in initial_top_idxs
        ]
        # 4. Trier et prendre les meilleurs selon args.final
        weighted_sorted = sorted(weighted, key=lambda x: x[1], reverse=True)
        final_top_idxs = [i for i, _ in weighted_sorted[:min(args.final, len(weighted_sorted))]]
